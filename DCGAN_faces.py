...
# set to 0.8. Perhaps leave out.

# In GANs, the recommendation is to not use pooling layers, and instead use the stride.
...
# images in this dataset are 218 x 178 x 3. We will first detect faces and then resize to 64 x 64 x 3.
def load_images():
...
for i in range(16):
    ...
    pyplot.axis("off")
    pyplot.imshow(dataset[random.randint(1,len(dataset)-1)])
pyplot.show()
